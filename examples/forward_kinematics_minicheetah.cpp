...
Eigen::VectorXd generateRandomJointConfig(raisim::ArticulatedSystem *robot,
  ...
  // Sample a random unit quaternion for indices 3–6
  {
    ...
    q.normalize();
    jointConfig.segment<4>(3) = q; // qw, qx, qy, qz
  }

  // Sample remaining joints (legs)
  for (int i = 7; i < jointConfig.size(); ++i) {
    std::uniform_real_distribution<double> dis(limits[i][0], limits[i][1]);
    jointConfig[i] = dis(gen);
  }
